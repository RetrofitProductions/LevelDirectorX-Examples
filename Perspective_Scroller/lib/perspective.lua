...
lib_perspective.createView = function(layerCount)
	...
	internal.processViewpoint = function()
		if internal.damping ~= view.damping then 
			internal.trackingLevel = 1 / view.damping 
			print (internal.trackingLevel)
			internal.damping = view.damping 
		end
	...
	function view:trackFocus()
		...
		for i = -prependedLayers + 1, #layers do
			-- if (layers[i].xParallax ~= 0 or layers[i].yParallax ~=0) then
	...
	function view:setParallax(...) 
		...
		for i = 1, #arg do 
			if type(arg[i]) == "table" then 
				layers[i].xParallax, layers[i].yParallax = arg[i][1], arg[i][2] 
			else 
				layers[i].xParallax, layers[i].yParallax = arg[i], arg[i] 
			end 
		end 
	end
	...
	function view:getViewXY() return internal.getViewXY() end
	------------------------------------------------------------------------------
	-- Build Layers
	------------------------------------------------------------------------------
